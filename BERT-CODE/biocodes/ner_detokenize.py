...
def transform2CoNLLForm(golden_path, output_dir, bert_pred, debug):
    ...
    with open(golden_path,'r', encoding='windows-1252') as in_:
    # with open(golden_path,'r') as in_:
        ...
        # print("--------------------------------------------------------------")
        # print(list(zip(ans['toks'], ans['labels'])))
        # print(len(ans['toks']))
        ...
        # print(list(zip(ans['toks'], ans['labels'])))
        # while '[CLS]' in bert_pred['toks']:
        #   bert_pred['toks'].remove('[CLS]')
        # ans['toks'] = [e for e in ans['toks'] if e not in ('[CLS]', '[SEP]')]
        ans_toks_label = [e for e in list(zip(ans['toks'], ans['labels'])) if e[0] not in ('[CLS]', '[SEP]')]
        ans['toks'] = [e[0] for e in ans_toks_label]
        ans['labels'] = [e[1] for e in ans_toks_label]
        
        bert_toks_label = [e for e in list(zip(bert_pred['toks'], bert_pred['labels'])) if e[0] not in ('[CLS]', '[SEP]')]
        bert_pred['toks'] = [e[0] for e in bert_toks_label]
        bert_pred['labels'] = [e[1] for e in bert_toks_label]
        ...
        # print(list(zip(bert_pred['toks'], bert_pred['labels'])))
        print(bert_pred['toks'])
        print(len(bert_pred['toks']))
        print(len(bert_pred['labels']))

        # for bt, at, al, bl in list(zip(bert_pred['toks'], ans['toks'], ans['labels'], bert_pred['labels'])):
        #     print(bt, at, al + "-MISC", bl + "-MISC")
        print("--------------------------------------------------------------")
    if debug:
        ansCount = 0
        prdCount = 0 
